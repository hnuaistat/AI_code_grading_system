...
- **항목 분할 시 배점 보존 (CRITICAL):** 점수가 명시된 조건 하나를 여러 세부 항목으로 나누는 경우, 나눈 항목들의 `score` 합계가 **반드시 원래 명시된 점수와 같아야** 합니다.
  - ✅ 예: `(1) 각 값을 8비트 이진수로 표현하시오. (1점)` → a 표현 0.5점 + b 표현 0.5점 (합계 1점 유지)
  - ✅ 예: `(3) 1행 3열 출력 후 저장하시오. (1점)` → subplot 배치 0.5점 + savefig 저장 0.5점 (합계 1점 유지)
  - ❌ 절대 금지: 나눈 각 항목에 원래 점수를 그대로 부여 (1점 조건 → 1점 + 1점 = 2점)
- **소문제 번호 표기 유지 (CRITICAL):** 문제 설명에 `(1)`, `(2)` 같은 소문제 번호가 있으면, 그 조건에서 나온 **모든** 항목의 `item` 텍스트 맨 앞에 해당 번호를 그대로 붙이세요. 하나의 번호 조건을 여러 항목으로 분할해도 모든 분할 항목에 같은 번호를 붙입니다.
  - ✅ 예: `(3) 1행 3열 출력 후 저장 (1점)` →
    - `(3) plt.subplot(1,3,n)으로 1행 3열 배치` 0.5점
    - `(3) plt.savefig('Q03.png')로 파일 저장` 0.5점
  - ❌ 나쁜 예: `plt.subplot(1,3,n)으로 1행 3열 배치` (번호 누락 — 어느 소문제인지 알 수 없음)
...
- **검산 필수:** JSON을 출력하기 직전에 각 문항의 `partial_score_criteria` score 합계를 계산하여
  `full_score`와 일치하는지 확인하세요. 합계가 `full_score`를 **초과하는 것은 절대 금지** —
  초과하면 분할된 항목들의 점수를 줄여서 합계를 정확히 맞춘 후 출력하세요.
